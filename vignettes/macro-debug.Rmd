...
command line. The normal output looks like this:
```{r eval=FALSE, echo=TRUE}
...
Now let's turn debugging on to examine the debug output. To turn debugging on,
...
Note the following about the above debug information:
  ...
  * The pre-processor then steps through the input file one line at a time.
  ...
  the line is not emitted to the output file, the line number is shown as blank.
  ...
  * The generated code is source and echoed into the execution section of the
  debug info.
...
encountered. The error encountered is displayed in the debug info, and the 
...
generated code, an error message was produced.  This printout gives
