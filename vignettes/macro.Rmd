...
Here are some more of the major difference between the SAS macro language and the 
R macro language:

  * R macro statements do not end with a semicolon (";").  They are single-line
  commands, and end with a carriage return.
  * R macro variables do not have a leading ampersand ("&").  They do have a trailing
  dot (".").
  * At present, there are no user-defined macro functions in the **macro** package.  
  To create a function, use the Base R function syntax.  
...
To use the **macro** package, you will first write your macro-enabled R code in a regular ".R" code file.  You can use
...
5. **Macro Functions**: A small number of macro functions
...
replaced exactly as they were assigned.  Macro variable expressions are not evaluated
unless done so explicitly using `%sysfunc()`. Usage of this macro function
...
been removed. Only the code inside TRUE condition has been emitted.
...
There are a couple macro functions that are considered necessary for proper
operation of the system.  The essential functions are `%sysfunc()` and 
`%symexist()`.
...
without quotes and without the "." suffix.  
