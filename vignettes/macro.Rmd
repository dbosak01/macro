...
Say something about shortcuts and interactive usage/addin.
...
Show simple but comprehensive example.  User-defined function with do loop, 
conditionals, and at least one built-in function.
...
Explain symbol table and symbol table functions.  Link to symbol table vignette.

### Macro Debugging

Explain debugging and demonstrate.  Link to debugging vignette.


### Next Steps

Link to all vignettes.
...
Macro conditionals allow you to conditionally execute a chunk of code.  If
...
b <- c('one', 'two', 'three')
```
As you can see, the `%symput()` function provides a way to pass values back and
forth between Base R and the macro language seamlessly.  This feature gives you 
the capability to make decisions in pre-processing
...
User-defined macro functions allow you to wrap up a set up statements,
