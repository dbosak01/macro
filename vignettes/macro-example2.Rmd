...
Note the following in the above driver:
  ...
  * Macro variables can be set in regular R code.  One stipulation is
  that they must be set before `msource()` is called. Also, the variable names
  ...
  * The above approach allows you to execute `msource()` directly
  ...
  line. Instead, you can run it from the "Source" or "Run" button in your
...
[  21][    ]: #%end 
[  22][  18]:  
[  23][  19]: # Create report object 
[  24][  20]: rpt <- create_report(out_pth, font = "Courier", output_type = "PDF") |> 
[  25][  21]:   page_header("Sponsor: Acme", "Study: ABC") |> 
[  26][  22]:   titles(c('Listing 1.0', 'SDTM Demographics')) |> 
[  27][  23]:   add_content(tbl, align = "left") |> 
[  28][  24]:   footnotes('Program: l_dm.R') |> 
[  29][  25]:   page_footer(Sys.time(), "CONFIDENTIAL", "Page [pg] of [tpg]") 
[  30][  26]:  
[  31][  27]: # Write report to file 
[  32][    ]: #%if (&preview. == TRUE) 
[  33][  28]: write_report(rpt, preview = 1) 
[  34][    ]: #%else 
[  35][    ]: write_report(rpt) 
[  36][    ]: #%end 
******************************************************************************** 
...
- page_footer: left=2025-10-26 10:36:41.852507 center=CONFIDENTIAL right=Page [pg] of [tpg]
- content: 
