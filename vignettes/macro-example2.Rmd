...
Now let's look at a more complicated example. For this example,
...
These parameters should be sourced and available in the environment 
before the `msource()` function is run. Any macro variables in the environment
will be copied to the pre-processor environment at the beginning of execution.

Another advantage of this approach is that you can source the macro driver
directly from the development environment. You do not need to run `msource()`
from the command line because it is called in the macro driver.
...
to pick which variables will end up in the final report.  The template table
...
In the above code, notice the following:
  ...
  * Analysis variable blocks are created using functions from the **procs**
  package.
  * The report code comes from an included code snippet.  
...
Here is the template code for reporting:
