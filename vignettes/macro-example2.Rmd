...
msource(file.path(base_dir., "templates/lst02.R"),
        ...
        debug = TRUE)
```
Note the following in the above example code:
  ...
  * The above approach allows you to execute `msource()` directly
  ...
  line, which is somewhat inconvenient.
  * The above call to `msource()` includes the `debug = TRUE` parameter. This 
  option will generate debug information in the console.  The debug results 
  will be shown below.
