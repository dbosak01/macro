...
#% <comment>
```
Where "&lt;comment&gt;" is any text string. 

The macro comment is a single line comment.  If you need multiple lines,
use multiple macro comments.
...
Here is a basic macro assignment of numeric values.  Note that either 
the left arrow assignment operator or equals sign are valid:
```{r eval=FALSE, echo=TRUE}
...
```
Note that the above code will generate an error, as the `y <- b.` runs
...
z <- c.
```
When the "temp1.R" file is resolved, it will look like this:
```{r eval=FALSE, echo=TRUE}
# Print macro variable
z <- 3
```
Note that the macro variable `c.` is replaced normally. The reason is because 
...
As long as they exist in the parent environment, the macro assignments will be
resolved in the pre-processor.
...
The conditional expressions to be evaluated should be regular R expressions,
...
The conditional expression may contain macro variables.  These macro variables
...
If an expression in a macro "if" block are TRUE, the code inside that block
...
```
The macro selected the "dev" path as indicated, constructed the path appropriately, 
and removed all macro statements.
...
#%include "<path>"
```
Where "&lt;path&gt;" is a path to the file to include. 
...
The macro include actually copies the code into the generated file.  The 
...
and "&lt;format&ft;" is an optional format code.  The expression may
contain R functions, operators, hard-coded values, and macro variables.
If "&lt;format&ft;" is supplied, the function will apply the format after 
the expression is resolved, and return the formatted value.
...
The `%sysfunc()` and `%symexist()` function are helper functions that are usually
...
Below is a simple example checking for the existance of some macro variables
