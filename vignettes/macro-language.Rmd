...
still end up with a very simple result.
...
and "&lt;format&gt;" is an optional unquoted format code.  The expression may
...
z <- 8
```
Observe the different ways each expression resolves depending on whether
`%sysfunc()` is used.
