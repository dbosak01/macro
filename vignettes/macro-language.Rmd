...
masks the expression to prevent normal resolution, which is desirable in 
some cases.
...
name instead of the resolved value, which can be desirable in some cases.
...
```
Why didn't `&x.` resolve?  Because `%nrstr()` prevented resolution.  This 
