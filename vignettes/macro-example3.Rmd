...
In this example, we will assign macro parameters directly in R code. 
...
Note the following in the above driver:
  ...
  * Macro variables can be set in regular R code.  One stipulation is
  ...
  must have a leading ampersand.  These variables will be copied to the macro
  symbol table when `msource()` starts.
